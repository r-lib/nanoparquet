...
static PyObject *nanoparquet_read(PyObject *self, PyObject *args) {
	const char *fname;
			...
			for (size_t col_idx = 0; col_idx < ncols; col_idx++) {
				...
				for (uint64_t row_idx = 0; row_idx < rc.nrows; row_idx++) {
					...
					if (!col.defined.ptr[row_idx]) {
						...
						case parquet::Type::FIXED_LEN_BYTE_ARRAY: { // oof, TODO clusterfuck
              ...
              auto value = ((pair<uint32_t, char *>*)col.data.ptr)[row_idx].second;
							current_item = PyUnicode_DecodeUTF8(value, len, nullptr);
							break;
						}
						default: {
...
static PyMethodDef parquet_methods[] = {
    { "read", nanoparquet_read, METH_VARARGS, "Read a parquet file from disk." },

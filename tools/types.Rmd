...
1. `INT96` is read as a `POSIXct` read vector with the `tzone` attribute
   ...
   `INT96` objects.
1. The `DECIMAL` converted type (`FIXED_LEN_BYTE_ARRAY` type) is read
...
1. Otherwise `BYTE_ARRAY` is read a character vector (`STRSXP`).

Other logical and converted types are read as their annotated low level
types:

1. `ENUM` is read as a character vector (`STRSXP`).
1. `UUID` is a fixed length byte array, and nanoparquet cannot read it
   currently.
...
1. `INT(64, false)` is read as a real vector (`REALSXP`). Large
   positive values may overflow into negative values, this is a known
   issue that we will fix.
1. `FLOAT16` is a fixed length byte array, and nanoparquet cannot read it
   currently.
1. `INTERVAL` is a fixed length byte array, and nanoparquet cannot read
   it currently.
1. `JSON` and `BSON` are read as character vectors (`STRSXP`).
1. Nested types (`LIST`, `MAP`) and the `UNKNOWN` logical types are
   not supported currently.

You can use the `parquet_columns()` function to see how R would read
...
- It uses it to detect `difftime` ofjects. Without the arrow metadata

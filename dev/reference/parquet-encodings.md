...
- Definition levels always use `RLE`. (Nanoparquet does not currently
  write repetition levels, but they'll also use `RLE`, once
  implemented.)
...
This encoding combines run-length encoding and bit-packing. Repeated
sequences of the same value can be run-length encoded, and non-repeated
parts are bit packed. It is used for data pages of dictionaries. The
dictionary pages themselves are `PLAIN` encoded.
...
header. It combines run-length encoding and bit packing. It is used for
`BOOLEAN` columns, and also for definition and repetition levels.
...
used for encoding definition and repetition levels. It is not used in
new Parquet files because the the `RLE` encoding includes it and it is
better.
...
between consecutive elements are small. The extreme case of an
arithmetic sequence can be encoded in O(1) space.
...
on how to select a non-default encoding when writing Parquet files.
